...
# load config
@hydra.main(config_path="../conf/supervised_tasks/", config_name=None, version_base=None)
def run_experiment(cfg: DictConfig) -> None:
    ...
    if cfg.params.criterion == "cross_entropy":
        criterion = CrossEntropyLoss(
            ...
            weight=torch.tensor(cfg.params.class_weights).to(device)
        ...
        criterion = BCEWithLogitsLoss(
            class_weights=torch.tensor(cfg.params.class_weights).to(device)
            if cfg.params.class_weights is not None
            else None
        )
    ...
    if cfg.params.mode == "train":
        # train
        trainer.train(
            train_loader, val_loader, test_loader, cfg.params.epochs, cfg.params.load_checkpoint
        )
